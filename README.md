...
1. 用 `visual_extract.py` 处理样例图片。
2. 检查 `visual_packet.json` 和 `annotated_regions.png`。
3. 让文字类大语言模型只基于视觉语义包回答。
4. 让多模态大语言模型基于原图、标注图和视觉语义包回答。
5. 与不使用本 Skill 的多模态回答做对照。
6. 根据失败案例补充常见误区和模板。
7. 把回归案例加入 `evals/`。
